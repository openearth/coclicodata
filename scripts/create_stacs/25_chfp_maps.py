...
BUCKET_PROJ = "coclico"
PROJ_NAME = "cfhp_all_stats/map_stats"
...
COLLECTION_ID = "cfhp_all_maps"  # name of stac collection
...
ds_path = ds_dir.joinpath("WP4", "front_end_data", "map_stats")
parq_dirs = ds_path.joinpath("maps")
ds_fp = ds_path.parent.joinpath("cfhp_all_stats.parquet")  # file directory dummy

# Front end makes geopackages to go alongside the parquet data
# if this exists define here
# FE_gpkg_fp = ds_path.joinpath("GCF_open_CBA_country_EPSG4326.gpkg")

# # load metadata template
metadata_fp = ds_path.parent.joinpath("metadata_cfhp_all_stats.json")
...
def create_collection(
    ...
    #     "geoserver_link",
    #     pystac.Asset(
    #         "https://coclico.avi.deltares.nl/geoserver/gwc/service/wmts?REQUEST=GetTile&SERVICE=WMTS&VERSION=1.0.0&LAYER=cfhp_all_stats:cfhp_all_stats&STYLE=&TILEMATRIX=EPSG:900913:{z}&TILEMATRIXSET=EPSG:900913&FORMAT=application/vnd.mapbox-vector-tile&TILECOL={x}&TILEROW={y}",
...
if __name__ == "__main__":
    ...
    # %% get descriptions
    uri_dum = f"gs://{BUCKET_NAME}/{BUCKET_PROJ}/cfhp_all_stats"
    paths_dum = fs.glob(uri_dum + "/*.parquet")
    uris_dum = ["gs://" + p for p in paths_dum]
    HREF_PREFIX_dum = urljoin(
        GCS_PROTOCOL, BUCKET_NAME, BUCKET_PROJ, "cfhp_all_stats"
    )  # cloud export directory
    ...
    for map_type in map_types:
        for rp in rps:
            for scen in scenarios:
                for time in times:
                    file_name = f"cfhp_all_stats_{map_type}_{rp}_{scen}_{time}.parquet"
